...
auto XTCP::bind(unsigned int port) -> bool {
  ...
  if (::bind(impl_->socked_fd_, reinterpret_cast<struct sockaddr *>(&sadder),
             sizeof(sadder)) < 0) {
    printf("Bind failed.\n");
    return false;
  }
...
auto XTCP::accepct() -> XTCP {
  ...
  auto client_fd = ::accept(impl_->socked_fd_,
                            reinterpret_cast<sockaddr *>(&cadder), &cadderlen);
  if (client_fd < 0) {
...
auto XTCP::connect(const char *ip, unsigned short port, int timeoutms) -> bool {
  ...
  fd_set set;
  if (::connect(impl_->socked_fd_, reinterpret_cast<struct sockaddr *>(&sadder),
                sizeof(sadder)) < 0) {
    FD_ZERO(&set);
...
auto XTCP::setBlock(bool is_block) -> bool {
  ...
  printf("Socket %d set to %s mode\n", impl_->socked_fd_,
         is_block ? "blocking" : "non-blocking");
  return true; // 返回成功状态
}

auto XTCP::getfd() const { return impl_->socked_fd_; }
